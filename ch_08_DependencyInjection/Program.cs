...
builder.Services.AddSingleton<BookService>(); // DI kaydı yapılıyor.
...
{
    return bookService.Books.Count > 0 ? Results.Ok(bookService.Books) : Results.NotFound();
});
...
{
    var books = string.IsNullOrEmpty(title) ? bookService.Books : bookService.Books.Where(b => b.Title != null && b.Title.ToLower().Contains(title.ToLower())).ToList();
