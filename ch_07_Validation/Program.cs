...
app.MapPost("/api/books", (Book? newBook) =>
{
    ...
    return Results.UnprocessableEntity(validationResult); // 422
});
app.MapPut("/api/books/{id:int}", (int id,Book? updateBook) =>
{
    ...
    {
        throw new ValidationException(validationResult.First().ErrorMessage);
    }
    
    string st = "";
    for (int i = 0; i < validationResult.Count; i++)
    {
        st = validationResult[i].ErrorMessage +" , "+st;
    }
...
app.MapGet("/api/books/search", (string? title,string? dsc) =>
{
